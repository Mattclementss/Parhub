...
export default function SignUpPage() {
  ...
  return (
    <div className="bg-[#1a2e1d] rounded-2xl border border-[#2a3d2c] p-6">
      <h2 className="text-xl font-black text-white mb-6">Create an account</h2>

      {state?.message && (
        <div className="mb-4 rounded-xl bg-[#1a3d1a] border border-[#2a5a2a] px-4 py-3 text-sm text-[#4ade80]">
          {state.message}
        </div>
      )}
      ...
      <p className="mt-5 text-center text-sm text-[#555]">
        ...
        <Link href="/signin" className="font-bold text-[#4ade80] hover:text-white transition-colors">
          Sign in
        </Link>
      </p>
    </div>
